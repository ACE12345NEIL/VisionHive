...
class ZoneStateManager:
    """Maps observations into zone state. Filters out unassigned zones without video tape/camera coverage."""

    def __init__(self):
        self.latest = {}
        self.environment = {}
        self.state = {'zones': [], 'assignment_method': 'camera_coverage'}
    ...
    def _zone_at(point, room):
        for zone in room.zones:
            ...
            if min(xs) <= point['x'] <= max(xs) and min(ys) <= point['y'] <= max(ys):
                return zone.id
        return None
    ...
    def update(self, frame_result, fusion_state):
        ...
        powers = assets['power_assumptions_w']
        mapping = assets['class_to_device_type']

        # Determine active zones that have a camera/video tape assigned
        active_assigned_zones = {z for z in assignments.values() if z}
        ...
        zones = {
            zone.id: {
                ...
                'people_count': 0,
                'person_ids': [],
                'activity_levels': {'stationary': 0, 'low': 0, 'medium': 0, 'high': 0},
                'device_counts': {},
                'device_load_w': 0.0,
                'light_count': 0,
                'lighting_load_w': 0.0,
                'temperature_c': self.environment.get('indoor_temperature'),
                'humidity_pct': self.environment.get('indoor_humidity')
            }
            for zone in room.zones
        }
        ...
        for person in fusion_state['people']:
            ...
            if zone_id and zone_id in zones:
                zone = zones[zone_id]
                zone['people_count'] += 1
                zone['person_ids'].append(person['id'])
                for camera_id, track_id in zip(person['camera_ids'], person['source_track_ids']):
                    ...
                    if track:
                        zone['activity_levels'][track.activity.name.lower()] += 1
                        break
        ...
        for device in assets.get('manual_devices', []):
            if device.get('is_on') and device.get('zone_id') in zones:
                ...
                zone['device_counts'][kind] = zone['device_counts'].get(kind, 0) + 1
                zone['device_load_w'] += device.get('power_w', powers.get(kind, 0))

        for light in assets.get('lights', []):
            if light.get('is_on') and light.get('zone_id') in zones:
                zone = zones[light['zone_id']]
                zone['light_count'] += 1
                zone['lighting_load_w'] += light.get('power_w', powers['light'])

        for zone in zones.values():
            zone['device_load_w'] = round(zone['device_load_w'], 1)
            zone['lighting_load_w'] = round(zone['lighting_load_w'], 1)

        # Filter out zones that do not have a video tape / camera assigned
        filtered_zones = [z for zid, z in zones.items() if zid in active_assigned_zones] if active_assigned_zones else list(zones.values())

        self.state = {
            'zones': filtered_zones,
            'assignment_method': 'homography_or_camera_coverage',
            'power_assumptions_note': assets['notes']
        }
        return self.state
